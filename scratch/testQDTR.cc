...
#include "ns3/aqua-sim-mobility-ug3.h"
#include <fstream>
#include <unistd.h>
#include <iostream>
#include <vector>
...
void DoWalk(NodeContainer node, int delayLeft) // Predict
{
	...
	for (NodeContainer::Iterator i = node.Begin(); i != node.End(); ++i)
	{
		...
		Ptr<UnderwaterGilderMobilityModel3> m_model = object->GetObject<UnderwaterGilderMobilityModel3>();
		...
		Ptr<UniformRandomVariable> m_openAngle = m_model->DoGetMOpenAngle();
		m_helper.UpdateWithBounds(m_bounds);
		double m_direction = m_model->DoGetMDirection();
		double m_pitch = m_model->DoGetMPitch();
		double min_depth = m_model->DoGetMinDepth();
		double max_depth = m_model->DoGetMaxDepth();
		...
		for (int i = 0; i <= delayLeft; i += 1)
		{
			...
			if (/*keepOriginalState == false && */ outOfDepth == true)
			{
				...
				while (count < 5)
				{
					...
					{
						// std::cout << "Get next pitch " << nextPitch * 180.0 / 3.14 << " ,openangle " << openAngle * 180.0 / 3.14 << " ,current position is " << position << "." << std::endl;
						break;
					}
					// std::cout << "Underwater gilder mobility do not find suitable pitch " << nextPitch * 180.0 / 3.14 << " ,openangle " << openAngle * 180.0 / 3.14 << "!" << std::endl;
					count++;
					if (count >= 5)
					{
						// std::cout << "Underwater gilder mobility can't find suitable pitch!" << std::endl;
						break;
					}
				}
				m_pitch = nextPitch;
...
{
	Config::SetDefault("ns3::UnderwaterGilderMobilityModel3::Speed", StringValue("ns3::UniformRandomVariable[Min=5.0|Max=10.0]"));
	Config::SetDefault("ns3::UnderwaterGilderMobilityModel3::Depth", StringValue("ns3::UniformRandomVariable[Min=100.0|Max=300.0]"));
	Config::SetDefault("ns3::UnderwaterGilderMobilityModel3::OpenAngle", StringValue("ns3::UniformRandomVariable[Min=30.0|Max=50.0]"));
	Config::SetDefault("ns3::UnderwaterGilderMobilityModel3::Bounds", StringValue("0|5000|0|5000"));
	...
	asHelper.SetRouting("ns3::AquaSimQDTR", "Width", DoubleValue(100), "TargetPos", Vector3DValue(Vector(1000, 1000, 4200)));
	...
	mobility.SetMobilityModel("ns3::UnderwaterGilderMobilityModel3",
	...
	AnimationInterface anim("road.xml");
	Simulator::Run();
	Simulator::Destroy();
