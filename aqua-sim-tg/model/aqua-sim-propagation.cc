	...
	*/
	...
	AquaSimPropagation::Rayleigh(double d, double f)
	{
		...
		*/
		...
		NS_LOG_DEBUG("Rayleigh dump: distance(km):" << d1 << ", k:" << k << ", f:" << f << ", a(f):" << alpha_f << ", A(l,f):" << t1 * t3);
		return t1 * t3;
	}
 ...
 */
	...
	AquaSimPropagation::Thorp(double dist, double freq)
	{
		...
		return spre + abso;
		*/
		return (0.11 * pow(freq, 2) / (1 + pow(freq, 2)) + 44 * pow(freq, 2) / (4100 + pow(freq, 2)) + 0.000275 * pow(freq, 2) + 0.0003);
	}
	...
	AquaSimPropagation::Thorp2(double dist, double freq)
	{
		...
		abso = dist / 1000 * (0.11 * pow(freq, 2) / (1 + pow(freq, 2)) + 44 * pow(freq, 2) / (4100 + pow(freq, 2)) + 0.000275 * pow(freq, 2) + 0.003);
		return spre + abso;
	}
