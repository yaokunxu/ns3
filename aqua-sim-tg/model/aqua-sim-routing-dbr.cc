...
std::cout << "2\n";
  ...
  m_beaconTimer->Schedule(Seconds(5));//xj
std::cout << "5\n";
...
AquaSimDBR::MakeBeacon(void)
{
  ...
  //ash.SetDirection(AquaSimHeader::DOWN);//xj
	//ash.SetSize(dbrh.Size() + IP_HDR_LEN);
  ast.SetDAddr(AquaSimAddress::GetBroadcast());
  ast.SetSAddr(AquaSimAddress::ConvertFrom(GetNetDevice()->GetAddress()));
  //ash->dport() = DBR_PORT;
...
AquaSimDBR::ForwardPacket(Ptr<Packet> p, int flag)
{
 ...
 // AquaSimHeader ash;
  ...
  NS_LOG_DEBUG("AquaSimDBR::DeadNeighb_Callback: node=" <<
      GetNetDevice()->GetAddress() << " is forwarding to " << ast.GetDAddr());
      //Address::instance().get_nodeaddr(iph->daddr()));

	// common settings for forwarding
//   ash.SetDirection(AquaSimHeader::DOWN);
// 	//ash->addr_type_ = AF_INET;
// 	ash.SetSize(dbrh.Size() + IP_HDR_LEN);//xj
  ptag.SetPacketType(AquaSimPtTag::PT_DBR);
      ...
      NS_LOG_DEBUG("AquaSimDBR::DeadNeighb_Callback: " <<
        ...
        //drop(p, DROP_RTR_TTL);
			...
			else
			{
				...
				dbrh.SetPrevHop(AquaSimAddress::ConvertFrom(GetNetDevice()->GetAddress()));
				dbrh.SetNumForward((dbrh.GetNumForward()+1));
  ...
  Simulator::Schedule(Seconds(delay),&AquaSimRouting::SendDown,this,
                        p,ast.GetNextHop(),Seconds(0));
	std::cout<< "PktID:"<<p->GetUid()<<" "<<"forward node:"<<AquaSimAddress::ConvertFrom(GetNetDevice()->GetAddress())<<"\n";
}
...
AquaSimDBR::BeaconIn(Ptr<Packet> p)
{
  ...
  AquaSimAddress src = dbrh.GetOwner();//改
...
bool AquaSimDBR::TxProcess(Ptr<Packet> p, const Address &dest, uint16_t protocolNumber) {
	/**
	 * 路由接受函数的判断逻辑如下:
	 * 1.加载路由表
	...
	DBRHeader dbrh;
	dbrh.SetNumForward((uint16_t)0);
	//dbrh.SetNHops(DBR_MAX_HOPS);
	dbrh.SetNHops(10);

	dbrh.SetOwner(AquaSimAddress::ConvertFrom(m_device->GetAddress()));
	dbrh.SetTarget(AquaSimAddress::ConvertFrom(dest));
	p->AddHeader(dbrh);
	//std::cout << "进入recv\n";
	//p->Print(std::cout);
  ...
  if(p->GetUid()>100){
			...
			outfile<<"pktID:"<<p->GetUid()<<" sendTime "<<curtime<<"\n";
			outfile.close();
	}
...
AquaSimDBR::Recv(Ptr<Packet> p, const Address &dest, uint16_t protocolNumber)
{
   ...
   std::ofstream outfile("NodeResiEnergy.txt", std::ios::app);
	...
	std::cout<< "PktID:"<<p->GetUid()<<" "<<"recv node:"<<AquaSimAddress::ConvertFrom(GetNetDevice()->GetAddress())<<"\n";
  //AquaSimHeader ash;
  DBRHeader dbrh;
  //Ipv4Header iph;
  AquaSimPtTag ptag;
 ...
 // p->AddHeader(ash);
  ...
  AquaSimAddress src = dbrh.GetOwner();
 AquaSimAddress dst = dbrh.GetTarget();//改
	//nsaddr_t src = Address::instance().get_nodeaddr(iph->saddr());
...
//	NS_ASSERT(!ash.GetErrorFlag());//xj
  ...
  Simulator::Schedule(Seconds(0),&AquaSimRouting::SendDown,this,
	...
	//std::cout << "1312312\n";
	//p->Print(std::cout);
	if (dst == GetNetDevice()->GetAddress())//dst == GetNetDevice()->GetAddress()改
	{// packet is for me
...
if(p->GetUid()>100){
				...
				delayT<<"pktID:"<<p->GetUid()<<" RecvTime "<<Simulator::Now().GetSeconds()<<"\n";
			    delayT.close();
	}
    NS_LOG_DEBUG("AquaSimDBR::Recv: address:" <<
...
AquaSimDBR::HandlePktForward(Ptr<Packet> p)
{
  ...
  p->RemoveTrailer(ast);
	...
	// common settings for forwarding
	dbrh.SetNumForward((dbrh.GetNumForward() +1));//xj
	//ash.SetDirection(AquaSimHeader::DOWN);
	//ash->addr_type_ = AF_INET;
	ptag.SetPacketType(AquaSimPtTag::PT_DBR);
	//ash.SetSize(dbrh.Size() + IP_HDR_LEN);
	ast.SetNextHop(AquaSimAddress::GetBroadcast());//xj
...
p->RemoveTrailer(ast);
  AquaSimAddress src = dbrh.GetOwner();//改
  AquaSimAddress dst = dbrh.GetTarget();
	//nsaddr_t src = Address::instance().get_nodeaddr(iph->saddr());
	//nsaddr_t dst = Address::instance().get_nodeaddr(iph->daddr());
