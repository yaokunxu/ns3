...
namespace ns3
{
	...
	{
		NS_LOG_INFO("RTSTimeoutHandler: node " << m_device->GetNode() << " timeout " << m_rtsTimeoutNum << " times");
		m_rtsTimeoutNum++;
...
*/
   ...
   ====================================================================== */
	...
	AquaSimTMac::AquaSimTMac()
	{
		...
		m_taDuration = (m_contentionWindow + m_maxShortPacketTransmissionTime + m_maxPropagationTime * 2) * 1.5;
...
*/
	...
	AquaSimTMac::GenerateSYN()
	{
		...
		NS_LOG_INFO("GenerateSYN: node(" << synh.GetSenderAddr() << ") generates SYN packet at " << Simulator::Now().GetSeconds());
		return pkt;
	}
	...
	AquaSimTMac::SendSYN()
	{
		...
		pkt->AddHeader(ash);
		pkt->AddPacketTag(ptag);
		NS_LOG_INFO("SendSYN:node(" << m_device->GetNode() << ") send SYN packet at " << Simulator::Now().GetSeconds());
	...
	AquaSimTMac::SendND(int pkt_size)
	{
		...
		NS_LOG_INFO("SendND:node(" << m_device->GetNode() << ") send ND type is " << ptag.GetPacketType() << " at " << Simulator::Now().GetSeconds());
		TxND(pkt, m_ndWindow);
	}
	...
	AquaSimTMac::SendShortAckND()
	{
		...
		if (m_arrivalTableIndex == 0)
			return; // not ND received
	...
	AquaSimTMac::ProcessShortACKNDPacket(Ptr<Packet> pkt)
	{
		...
		{
			NS_LOG_INFO("ProcessNDPacket:node(" << myaddr << ") to node (" << m_shortLatencyTable[i].node_addr << ") short latency is " << m_shortLatencyTable[i].latency << " and number is " << m_shortLatencyTable[i].num);
		}
		return;
	}
	...
	AquaSimTMac::ProcessSYN(Ptr<Packet> pkt)
	{
		...
		if (d >= 0.0)
		{
			while (d >= m_periodInterval)
				d -= m_periodInterval;
		}
		else
		{
			while (d + m_periodInterval <= 0.0)
				d += m_periodInterval;
		}
		...
		{
			NS_LOG_INFO("ProcessSYN: node(" << m_device->GetAddress() << ") to node (" << m_periodTable[i].node_addr << ") period difference is " << m_periodTable[i].difference);
		}
...
*/
	...
	AquaSimTMac::Wakeup()
	{
		...
		if (m_macStatus == TMAC_SLEEP)
		{
			...
			if (NewData())
			{
				NS_LOG_INFO("WakeUp: There is new data in node " << m_device->GetAddress() << " and the number of packet is " << m_txbuffer.num_of_packet);
				SendRTS();
			}
		}
		return;
	...
	AquaSimTMac::ReStart()
	{
		...
		if ((m_macStatus == TMAC_SILENCE) || (m_macStatus == TMAC_IDLE))
		{
			...
			if (NewData())
			{
				NS_LOG_INFO("Restart: There is new data in node " << m_device->GetAddress() << " and the number of packet is " << m_txbuffer.num_of_packet);
				SendRTS();
			}
		}
		return;
	...
	AquaSimTMac::PrintTable()
	{
		...
		for (int i = 0; i < T_TABLE_SIZE; i++)
		{
			NS_LOG_INFO("Node addr is " << m_shortLatencyTable[i].node_addr << " and short latency is " << m_shortLatencyTable[i].latency);
		}
		...
		for (int i = 0; i < T_TABLE_SIZE; i++)
		{
			NS_LOG_INFO("Node addr is " << m_periodTable[i].node_addr << " and difference is " << m_periodTable[i].difference);
		}
	}
	...
	AquaSimTMac::ProcessSilence()
	{
		...
		NS_LOG_INFO("ProcessSilence: node " << m_device->GetAddress() << ": there still exists silence record..");
	...
	AquaSimTMac::CleanSilenceTable()
	{
		...
		while (i < m_silenceTableIndex)
		{
			...
			{
				NS_LOG_INFO("CleanSilence: node " << m_device->GetAddress() << " clears the silence record...");
	...
	AquaSimTMac::DeleteSilenceRecord(AquaSimAddress node_addr)
	{
		int index = -1;
		for (int i = 0; i < m_silenceTableIndex; i++)
			if (m_silenceTable[i].node_addr == node_addr)
				index = i;

		if (index != -1)
			DeleteSilenceTable(index);
		return;
	}
	...
	AquaSimTMac::ConfirmSilenceTable(AquaSimAddress sender_addr, double duration)
	{
		int index = -1;
		for (int i = 0; i < m_silenceTableIndex; i++)
			if (m_silenceTable[i].node_addr == sender_addr)
				index = i;

		if (index != -1)
			m_silenceTable[index].confirm_id = 1;
		else
		{
	...
	AquaSimTMac::SendRTS()
	{
		...
		{
			NS_LOG_INFO("SendRTS: node " << m_device->GetAddress() << " is in TMAC_SILENCE state at " << Simulator::Now().GetSeconds());
			return;
		}
		...
		NS_LOG_INFO("SendRTS: node " << m_device->GetAddress() << "is in TMAC_RTS at " << Simulator::Now().GetSeconds() << " will tx RTS in " << t2);
	...
	AquaSimTMac::TxRTS(Ptr<Packet> pkt, AquaSimAddress receiver_addr)
	{
		...
		if (m_macStatus == TMAC_SILENCE)
		{
			NS_LOG_INFO("TxRTS:node(" << m_device->GetNode() << ") is at silence state quit " << Simulator::Now().GetSeconds());
			m_rtsTimeoutNum = 0;
			pkt = 0;
			return;
		}
		...
		if (NIDLE == status)
		{
			...
			NS_LOG_INFO("TxRTS, node " << m_device->GetAddress() << " Tx RTS at " << Simulator::Now().GetSeconds() << " and timeout is " << t << " number of try is " << m_rtsTimeoutNum);
			TStatusHandler_SetStatus(NIDLE);
			// m_rtsTimeoutEvent.Cancel();
			m_statusEvent = Simulator::Schedule(txtime, &AquaSimTMac::TStatusHandler, this);
			m_rtsTimeoutNum = 0;
		...
		if (RECV == status)
		{
			NS_LOG_INFO("TxRTS, node " << m_device->GetAddress() << " is in RECV state, backoff...");
	...
	AquaSimTMac::ProcessCTSPacket(Ptr<Packet> pkt)
	{
		...
		if (receiver_addr != m_device->GetAddress())
		{
			if (t > 0)
			{
				...
				if (m_macStatus == TMAC_SILENCE)
				{
					NS_LOG_INFO("ProcessCTS: node(" << m_device->GetNode() << ") I am already in silence state");
					if (m_lastSilenceTime < t + Simulator::Now().ToDouble(Time::S))
					{
						NS_LOG_INFO("ProcessCTS: node(" << m_device->GetNode() << ") the silence is longer than existing one...");
						m_silenceEvent.Cancel();
						m_silenceEvent = Simulator::Schedule(Seconds(t), &AquaSimTMac::ProcessSilence, this);
				...
				{
					NS_LOG_INFO("ProcessCTS: node(" << m_device->GetNode() << ") I am going to be in silence state");
	...
	AquaSimTMac::ClearTxBuffer()
	{
		...
		for (int i = 0; i < MAXIMUM_BUFFER; i++)
		{
			...
			if (m_bitMap[i] == 1)
				m_txbuffer.DeletePacket(p1[i]);
		}

		NS_LOG_INFO("ClearTxBuffer:m_txbuffer is cleared, there are " << m_txbuffer.num_of_packet << "packets left");
	...
	AquaSimTMac::ProcessACKDataPacket(Ptr<Packet> pkt)
	{
		...
		{
			NS_LOG_INFO("ProcessACKDATAPacket:node(" << m_device->GetNode() << " not in transmission state, just quit");
			pkt = 0;
			return;
		}

		NS_LOG_INFO("ProcessACKDATAPacket:node(" << m_device->GetNode() << " cancel timeout duration=" << m_duration);
		...
		NS_LOG_INFO("ProcessACKDATAPacket:node(" << m_device->GetNode() << "received the bitmap is:");

		for (int i = 0; i < MAXIMUM_BUFFER; i++)
			NS_LOG_INFO("bmap[" << i << "]=" << m_bitMap[i]);

		NS_LOG_INFO("ProcessACKDATAPacket: m_txbuffer will be cleared, there are " << m_txbuffer.num_of_packet << " packets in queue and duration=" << m_duration);
	 ...
	 */
		...
		NS_LOG_INFO("ProcessACKDATAPacket: node " << m_device->GetNode() << " unlock m_txbuffer m_duration=" << m_duration);
		ResumeTxProcess();
	...
	AquaSimTMac::ProcessRTSPacket(Ptr<Packet> pkt)
	{
		...
		double t = 2 * m_maxPropagationTime + 2 * m_maxShortPacketTransmissionTime + 2 * m_minBackoffWindow;
		pkt = 0;
		...
		if ((receiver_addr != m_device->GetAddress()) && (silenceTime > 0))
		{
			...
			if (m_macStatus == TMAC_IDLE)
			{
				NS_LOG_INFO("ProcessRTS:node(" << m_device->GetNode() << ") I am not the intended receiver and will be in silence");
			...
			if (m_macStatus == TMAC_SILENCE)
			{
				...
				if (m_lastSilenceTime < t + Simulator::Now().ToDouble(Time::S))
				{
					NS_LOG_INFO("ProcessRTS:node(" << m_device->GetNode() << ") I am not the intended receiver, gets a longer silence...");
			...
			if (m_macStatus == TMAC_RTS)
			{
				...
				{
					NS_LOG_INFO("ProcessRTS:node(" << m_device->GetNode() << ") I am not the intended receiver and quits the  RTS state");
					InsertSilenceTable(sender_addr, silenceTime);
					m_macStatus = TMAC_SILENCE;
					m_poweroffEvent.Cancel();
					m_rtsTimeoutEvent.Cancel();
			...
			NS_LOG_INFO("ProcessRTS:node(" << m_device->GetNode() << ") this RTS is not for me and I am in unknown state");
			return;
		}
		...
		if (m_macStatus == TMAC_IDLE)
		{
			NS_LOG_INFO("ProcessRTS:node(" << m_device->GetNode() << ") is in idle state and ready to process the RTS");
			double dt = 0;
	...
	AquaSimTMac::InsertSilenceTable(AquaSimAddress sender_addr, double duration)
	{
		...
		{
			NS_LOG_INFO("InsertSilenceTable:node(" << m_device->GetNode() << ") this silence from node " << sender_addr << " is new one, duration=" << duration << " at time " << Simulator::Now().GetSeconds());
		...
		{
			NS_LOG_INFO("InsertSilenceTable:node(" << m_device->GetNode() << ") this silence from node " << sender_addr << " is old one, duration=" << duration << " at time " << Simulator::Now().GetSeconds());
	...
	AquaSimTMac::TxCTS(Ptr<Packet> p)
	{
		...
		if (m_macStatus == TMAC_SILENCE)
		{
			NS_LOG_INFO("TxCTS:node " << m_device->GetNode() << ", I am in silence state, I have to quit..");
			m_ackEvent.Cancel();
			m_timeoutEvent.Cancel();
			m_ctsNum = 0;
			p = 0;
		...
		if (m_ctsNum == 2)
		{
			NS_LOG_INFO("TxCTS:node " << m_device->GetNode() << ", I have to try to send CTS twice, I have to quit..");
			m_ctsNum = 0;
			m_ackEvent.Cancel();
			p = 0;
			m_macStatus = TMAC_IDLE;
		...
		double l = CheckLatency(m_shortLatencyTable, receiver_addr);
		double t = 2.2 * l + m_minBackoffWindow * 2 + m_maxShortPacketTransmissionTime + m_maxLargePacketTransmissionTime;
		//         unused
		...
		if (NIDLE == status)
		{
			...
			NS_LOG_INFO("TxCTS:node" << m_device->GetNode() << " tx CTS " << Simulator::Now().GetSeconds() << " and timeout is set at " << t);
	...
	AquaSimTMac::ProcessDataPacket(Ptr<Packet> pkt)
	{
		...
		{
			if (m_macStatus == TMAC_SILENCE)
				DataUpdateSilenceTable(m_dataSender);
			else
			{
				NS_LOG_INFO("ProcessDataPacket: node " << m_device->GetNode() << ", I am not in silence state, my state is " << m_macStatus);
		...
		if (m_macStatus == TMAC_CTS)
			m_macStatus = TMAC_RECV;
		m_timeoutEvent.Cancel();

		MarkBitMap(num);
		NS_LOG_INFO("ProcessDataPacket: node " << m_device->GetNode() << " send up the packet");
		SendUp(pkt);
		// SendACKPacket();
		return;
	}
	...
	AquaSimTMac::SendACKPacket()
	{
		...
		NS_LOG_INFO("ScheduleACKData: Schdeule ACKDATA: node " << m_device->GetNode() << " return bitmap is");
		for (int i = 0; i < MAXIMUM_BUFFER; i++)
			NS_LOG_INFO("bmap[" << i << "]=" << m_bitMap[i]);
	...
	AquaSimTMac::TxACKData(Ptr<Packet> pkt)
	{
		...
		if (NIDLE == status)
		{
			NS_LOG_INFO("TxACKData node " << m_device->GetNode() << " is idle state at " << Simulator::Now().GetSeconds());
			//m_device->SetTransmissionStatus(SEND);
		...
		if (RECV == status)
		{
			NS_LOG_INFO("TxACKData node " << m_device->GetNode() << " is in recv state at " << Simulator::Now().GetSeconds() << " will be interrupted");
...
*/
	 ...
	 */
		...
		NS_LOG_INFO("TxData:node " << m_device->GetNode() << " TxData at time " << Simulator::Now().GetSeconds() <<
					/*" data type is " << hdr2->mess_type << */
					" packet data_num=" << datah.GetDataNum() << " class data_num=" << m_numData);
		TransStatus status = m_device->GetTransmissionStatus();
		...
		if (RECV == status)
		{
			NS_LOG_INFO("TxData:node " << m_device->GetNode() << " TxData interrupt the receiving status at " << Simulator::Now().GetSeconds());
		...
		if (m_txbuffer.IsEnd())
		{
			NS_LOG_INFO("Txdata:node " << m_device->GetNode() << " is in state MAC_TRANSMISSION");
			...
			NS_LOG_INFO("TxData:node " << m_device->GetNode() << " TxData at " << Simulator::Now().GetSeconds() << " and timeout is set " << t);
			m_timeoutEvent.Cancel();
			m_timeoutEvent = Simulator::Schedule(Seconds(t), &AquaSimTMac::SetIdle, this);
			m_numData = 0;
		}
		else
		{
			...
			NS_LOG_INFO("TxData:node " << m_device->GetNode() << " schedule next data packet, interval=" << it << " at time " << Simulator::Now().GetSeconds());
...
*/
	...
	AquaSimTMac::TxProcess(Ptr<Packet> pkt)
	{
		...
		NS_LOG_INFO("TxProcess:node " << m_device->GetNode() << " put new data packets in m_txbuffer");
		if (!m_txbuffer.IsFull())
			//if(callback_) callback_->handle(&m_statusEvent);
			return false;
		return true;
	...
	AquaSimTMac::RecvProcess(Ptr<Packet> pkt)
	{
		...
		if (ash.GetErrorFlag())
		{
			NS_LOG_INFO("RecvProcess:node " << m_device->GetNode() << " gets a corrupted packet from node " << receiver_addr << " at " << Simulator::Now().GetSeconds());
			return false;
		}
		NS_LOG_INFO("RecvProcess:node " << m_device->GetNode() << " gets a packet from node " << receiver_addr << " at " << Simulator::Now().GetSeconds());

		if (dst == AquaSimAddress::GetBroadcast())
		{
			if (ptype == PT_ND)
				ProcessNDPacket(pkt); //this is ND packet
			if (ptype == PT_SYN)
				ProcessSYN(pkt);
			return true;
		}
		...
		NS_LOG_INFO("RecvProcess:node " << m_device->GetNode() << " this is neither broadcast nor my packet " << dst << " , just drop it at " << Simulator::Now().GetSeconds());
		pkt = 0;
		return false;
	}
