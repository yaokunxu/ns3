...
class Disconnected_Timer : public Timer{
	friend class DN;
public:
...
class DN {
	friend class Disconnected_Timer;
public:
  AquaSimAddress first;
  AquaSimAddress second;
  Disconnected_Timer expire_time;
  void Expire();
 // void Expire(Disconnected_Timer *disconnected_Timer);
...
public:
 AquaSimDynamicRoutingTable(DN tp[100]);
...
public:
  ...
  int m_n;
  //const static int n=new const static int(n);

  //DN *tp[]=new DN[m_n];
  DN tp[100];
  bool TxProcess(Ptr<Packet> p, const Address &dest, uint16_t protocolNumber);
