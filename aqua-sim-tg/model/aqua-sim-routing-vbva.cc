...
int64_t
AquaSimVBVA::AssignStreams (int64_t stream)
{
  NS_LOG_FUNCTION (this << stream);
  m_rand->SetStream(stream);
  return 1;
}
bool AquaSimVBVA::TxProcess(Ptr<Packet> p, const Address &dest, uint16_t protocolNumber) {
	/**
	 * 路由接受函数的判断逻辑如下:
	 * 1.加载路由表
...
bool AquaSimVBVA::Recv(Ptr<Packet> packet, const Address &dest, uint16_t protocolNumber)
{
  ...
  if (vbh.GetNumForwards()==0)  //new packet
  {
     std::cout<<"vbh.GetNumForwards()==0"<<std::endl;
  ...
  else
  {
    ...
    ast.SetSAddr(vbh.GetSenderAddr());
    ast.SetDAddr(vbh.GetTargetAddr());
   // ash.SetSAddr(myAddr);
    vbh.SetNumForwards(vbh.GetNumForwards() + 1);
    packet->AddTrailer(ast);
    packet->AddHeader(vbh);
  }
 // std::cout<<"elsechu"<<std::endl;
...
//  unsigned int* packet_status=PacketStatusTable.GetHash(source,pkt_num);
      ...
      std::cout<<"processcentered"<<std::endl;
			return true;
		} else packet=0;
	}
...
//std::cout<<"expension /data"<<std::endl;
		unsigned int* packet_status=PacketStatusTable.GetHash(source,pkt_num);

		if((!packet_status)||((*packet_status)==SUPPRESSED)) {
      //std::cout<<"status sippressed"<<std::endl;
...
{
	//uint16_t protocol=m_protocol;
  std::cout<<"收到了"<<std::endl;
  if (!SendUp(pkt))
...
void AquaSimVBVA::ProcessBackpressureTimeout(Ptr<Packet> pkt)
{
 ...
 // AquaSimHeader ash;
  ...
  //pkt->AddHeader(ash);
    ...
    // NS_LOG_WARN("AquaSimVBVA: " << GetNetDevice()->GetAddress() <<
    //   " is an end node size is " << size);//xj
...
void AquaSimVBVA::SendDataTermination(const Ptr<Packet> p)
{
  ...
  //p->AddHeader(ash2);
     ...
     // ash.SetSize(m_controlPacketSize*8);
      ...
      ast.SetDAddr(AquaSimAddress::ConvertFrom(GetNetDevice()->GetAddress()));
      /*iph->src_=here_;
...
return NULL;
 ...
 // backpressure->RemoveHeader(ash);
  ...
  backpressure->RemoveTrailer(ast);
