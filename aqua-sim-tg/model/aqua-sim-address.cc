...
Address
AquaSimAddress::IntConvertTo(uint16_t addr)
{
	uint8_t other_address[2];
	other_address[0] = (addr >> 8) & 0xff;
	other_address[1] = (addr >> 0) & 0xff;
	  return Address (GetType (), other_address, 2);
...
AquaSimAddress
AquaSimAddress::IpToAuqaAddress (Ipv4Address& ipAddr)
{
  uint32_t ip = ipAddr.Get();
  uint16_t lastnum = ((ip >> 0) & 0xff);
  AquaSimAddress aquaAddr = AquaSimAddress (lastnum);
  std::cout<<aquaAddr<<std::endl;
  return aquaAddr;
}
Ipv4Address
AquaSimAddress::AuqaAddressToIp (AquaSimAddress& aquaAddr)
{
  int id = ConvertFrom(aquaAddr).GetAsInt();
  std::string iptemp = "10.1.1."+ std::to_string(id);
...
{
 // std::cout<<"Address ismatchingtype"<<std::endl;
