...
int SFamaHeader::GetSize(enum PacketType pType)
{
	int pkt_size = 2 * sizeof(AquaSimAddress); //source and destination addr in hdr_mac
	pkt_size += sizeof(uint16_t) + 1;		   //size of packet_type and slotnum
	return pkt_size;
}
void SFamaHeader::SetPType(uint8_t pType)
{
	m_pType = pType;
}
