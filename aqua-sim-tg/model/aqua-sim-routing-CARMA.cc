...
bool AquaSimCARMA::TxProcess(Ptr<Packet> packet, const Address &dest, uint16_t protocolNumber)
{
	...
	std::cout << "TxProcess"
			  << " sender address " << AquaSimAddress::ConvertFrom(GetNetDevice()->GetAddress()).GetAsInt() << std::endl;
...
bool AquaSimCARMA::Recv(Ptr<Packet> packet, const Address &dest, uint16_t protocolNumber)
{
	...
	{ // no headers //TODO create specalized Application instead of using this hack.// how many times this pkt was forwarded
		...
		vbh.SetPrevioushop(AquaSimAddress::ConvertFrom(GetNetDevice()->GetAddress()));
		vbh.SetForwardAddr(AquaSimAddress::ConvertFrom(GetNetDevice()->GetAddress()));
		vbh.SetPkNum((uint16_t)m_pkCount++);
		vbh.SetValue(V);
		// vbh.SetNexthopAddr();
	...
	vbh.Print(std::cout);
	AquaSimAddress forw = vbh.GetForwardAddr();
	std::cout << "forw" << forw.GetAsInt() << std::endl;

	if (vbh.GetForwardAddr() != AquaSimAddress::ConvertFrom(GetNetDevice()->GetAddress()))
	{
		PktlocalTable.PutInHash(vbh.GetForwardAddr(), vbh.GetValue(), vbh.GetPkNum(), vbh.GetP(AquaSimAddress::ConvertFrom(GetNetDevice()->GetAddress()).GetAsInt()));
		carma_neighbor *hash = PktlocalTable.GetHash(vbh.GetForwardAddr());
		hash->pji++;
	}

	std::cout << "Forward:" << vbh.GetForwardAddr() << std::endl;
...
void AquaSimCARMA::ConsiderNew(Ptr<Packet> pkt)
{
	...
	std::cout << "Curnode:" << AquaSimAddress::ConvertFrom(GetNetDevice()->GetAddress()).GetAsInt() << " receive pkr from node:"
...
void AquaSimCARMA::Retransmission(Ptr<Packet> pkt)
{
	...
	pkt->PeekHeader(vbh);
	std::cout << "n:" << (int)vbh.GetN() << std::endl;
