...
#include "aqua-sim-routing-QELAR.h"
...
NS_LOG_COMPONENT_DEFINE("AquaSimQELAR");
// NS_OBJECT_ENSURE_REGISTERED(AquaSimPktHashTable);
// todo add timeer
// clock_t start,finish;
AquaSimPktlocalTableQELAR::AquaSimPktlocalTableQELAR()
{
...
{
    NS_LOG_FUNCTION(this);
    for (std::map<local_entry, qelar_neighbor *>::iterator it = m_htable.begin(); it != m_htable.end(); ++it)
    {
        delete it->second;
    }
    m_htable.clear();
}
void AquaSimPktlocalTableQELAR::Reset()
{
    m_htable.clear();
}
qelar_neighbor *
AquaSimPktlocalTableQELAR::GetHash(AquaSimAddress forwarderAddr, Vector p)
{
    // std::cout<<"++++++GetHash+++++++++++++\n";
    local_entry entry = std::make_pair(forwarderAddr, p);

    std::map<local_entry, qelar_neighbor *>::iterator it;
...
void AquaSimPktlocalTableQELAR::PutInHash(AquaSimAddress fAddr, double Vvalue, Vector p, unsigned int Energy, unsigned int Avgenergy, unsigned int Density, unsigned int sendnum, unsigned int sendsucc)
{
    ...
    /*for(it=m_htable.begin();it!=m_htable.end();it++){
        ...
        if(cur.first==fAddr){
          ...
          unsigned int sndsucc=nei->sendsucc;
           std::map<local_entry,qelar_neighbor*>::iterator iter;
    ...
    std::pair<local_entry, qelar_neighbor *> newPair;
    newPair.first = entry;
...
void AquaSimPktlocalTableQELAR::UpdateHash(AquaSimAddress local, AquaSimAddress nexthop, double Vvalue)
{ // 更新V值和Sendnum
    std::map<local_entry, qelar_neighbor *>::iterator it;
    for (it = m_htable.begin(); it != m_htable.end(); it++)
    {
        local_entry update = it->first;
        qelar_neighbor *neibor = it->second;
...
void AquaSimPktlocalTableQELAR::UpdateSuccnum(AquaSimAddress previous, AquaSimAddress forward)
{
    std::map<local_entry, qelar_neighbor *>::iterator it;
    for (it = m_htable.begin(); it != m_htable.end(); it++)
    {
...
void AquaSimPktlocalTableQELAR::updateOwn(double avgengy, double denty, AquaSimAddress cur)
{
    std::map<local_entry, qelar_neighbor *>::iterator it;
    for (it = m_htable.begin(); it != m_htable.end(); it++)
    {
...
AquaSimDataHashTableSQELAR::AquaSimDataHashTableSQELAR()
{
    NS_LOG_FUNCTION(this);
    Reset();
...
void AquaSimDataHashTableSQELAR::Reset()
{
...
int *AquaSimDataHashTableSQELAR::GetHash(int *attr)
{
...
void AquaSimDataHashTableSQELAR::PutInHash(int *attr)
{
    // bool newPtr = true;
...
NS_OBJECT_ENSURE_REGISTERED(AquaSimQELAR);

AquaSimQELAR::AquaSimQELAR()
{
    ...
    /*Simulator::Schedule(Seconds(0.5),
                 &AquaSimQELAR::iniNodetable, this);*/
    Simulator::Schedule(Seconds(0.5),
                        &AquaSimQELAR::iniNodetable1, this);
    /*Simulator::Schedule(Seconds(10),
                  &AquaSimQELAR::PrepareMessage, this);*/
}

TypeId
AquaSimQELAR::GetTypeId(void)
{
    static TypeId tid = TypeId("ns3::AquaSimQELAR")
                            .SetParent<AquaSimRouting>()
                            .AddConstructor<AquaSimQELAR>()
                            .AddAttribute("HopByHop", "Hop by hop QLRP setting. Default 0 is false.",
                                          IntegerValue(0),
                                          MakeIntegerAccessor(&AquaSimQELAR::m_hopByHop),
                                          MakeIntegerChecker<int>())
                            .AddAttribute("EnableRouting", "Enable routing QLRP setting. Default 1 is true.",
                                          IntegerValue(1),
                                          MakeIntegerAccessor(&AquaSimQELAR::m_enableRouting),
                                          MakeIntegerChecker<int>())
                            .AddAttribute("Width", "Width of QELAR. Default is 100.",
                                          DoubleValue(3000),
                                          // DoubleValue(100),
                                          MakeDoubleAccessor(&AquaSimQELAR::m_width),
                                          MakeDoubleChecker<double>())
                            .AddAttribute("TargetPos", "Position of target sink (x,y,z).",
                                          Vector3DValue(),
                                          MakeVector3DAccessor(&AquaSimQELAR::m_targetPos),
                                          MakeVector3DChecker());
    return tid;
...
int64_t
AquaSimQELAR::AssignStreams(int64_t stream)
{
...
// 传输层调用Txprocess
bool AquaSimQELAR::TxProcess(Ptr<Packet> packet, const Address &dest, uint16_t protocolNumber)
{
    QELARHeader vbh;
    // start=clock();
...
// MAC层的包调用Recv
bool AquaSimQELAR::Recv(Ptr<Packet> packet, const Address &dest, uint16_t protocolNumber)
{
    NS_LOG_FUNCTION(this);
    std::cout << "================qelar Recv=================\n";
    ...
    // en.GetEnergy();
    // std::cout<<"qelar energy "<<en.GetEnergy();
    QELARHeader vbh;
    AquaSimTrailer tra;
    ...
    {
        // 收到信息包，更新表
        std::cout << "recv message DATA_READY on QELAR Recv\n";
        // 接到的包先将包信息插入(表中有信息则更新，没有就插入所有)
        // qelar_neighbor *hash=PktlocalTable.GetHash(vbh.GetSenderAddr(),vbh.GetExtraInfo().f);
        // todo
    ...
    if (!m_enableRouting)
    {
        ...
        {
            packet = 0;
            std::cout << "qelar recv vbh.GetMessType() != AS_DATA vbh.GetMessType():" << vbh.GetMessType() << std::endl;
            return false;
        }
    ...
    else
    {
        ...
        qelar_neighbor *hash = PktlocalTable.GetHash(vbh.GetForwardAddr(), vbh.GetExtraInfo().f);
        // todo
...
void AquaSimQELAR::ConsiderNew(Ptr<Packet> pkt)
{
    std::cout << "===========qelar: considerNew===========\n";
    NS_LOG_FUNCTION(this);
    ...
    NS_LOG_INFO("AquaSimQELAR::ConsiderNew: data packet");
    // todo 打印每个节点的剩余能量
    ...
    if (GetNetDevice()->GetAddress() == from_nodeAddr)
    {
        ...
        // MACprepareF(pkt);
        Simulator::Schedule(Seconds(0.5),
                            &AquaSimQELAR::MACprepareF, this, pkt);
        // MACsend(pkt,0.5);//macsend比Macprepare提前执行了
        Simulator::Schedule(Seconds(0.5), &AquaSimQELAR::MACsend, this, pkt, 0.5);
        // 发送完后更新自己的表中能量和平均能量值
        // todo
        // updateEnergyAfterSend(AquaSimAddress::ConvertFrom(GetNetDevice()->GetAddress()));
        Simulator::Schedule(Seconds(0.5), &AquaSimQELAR::updateEnergyAfterSend, this,
                            AquaSimAddress::ConvertFrom(GetNetDevice()->GetAddress()));
        // 把source节点的V值单独打印log
        Simulator::Schedule(Seconds(0.5),
                            &AquaSimQELAR::printSourceV, this, m_pkCount);
        // printSourceV();
        //}
        return;
    }
...
void AquaSimQELAR::Terminate()
{
    NS_LOG_DEBUG("AquaSimQELAR::Terminate: Node=" << GetNetDevice()->GetAddress() << ": remaining energy=" << GetNetDevice()->EnergyModel()->GetEnergy() << ", initial energy=" << GetNetDevice()->EnergyModel()->GetInitialEnergy());
}

void AquaSimQELAR::StopSource()
{
    /*
       Agent_List *cur;
...
void AquaSimQELAR::PrepareMessage()
{
    std::cout << "-----------qlr PrepareMessage-----------\n";
    Ptr<Packet> pkt = Create<Packet>();
    std::cout << "-----------qelar PrepareMessage Create pkt-----------\n";
    QELARHeader vbh;
    AquaSimTrailer ast;
    ast.SetModeId(1);
    ...
    {
        // std::pair<local_entry,qelar_neighbor*> curpair;
    ...
    Simulator::Schedule(Seconds(65.0),
                        &AquaSimQELAR::PrepareMessage, this); // 定期调用自己
                                                              // MACprepare(pkt);
                                                              // return pkt;
}

void AquaSimQELAR::MACprepareF(Ptr<Packet> pkt)
{
    std::cout << "-----------qelar MACprepareF-----------\n";
    QELARHeader vbh;
    AquaSimTrailer tra;
    pkt->RemoveTrailer(tra);
    ...
    for (it = PktlocalTable.m_htable.begin(); it != PktlocalTable.m_htable.end(); it++)
    {
        ...
        else
        {
            ...
            double Rt = Psucc * (-g + a1 * succEn + a2 * SuccAvgEn) + Pdefeat * (-g + b1 * defEn + b2 * DefeatAvgEn);
            // 计算V值,判断V值大小，找下一跳,
...
void AquaSimQELAR::MACprepare(Ptr<Packet> pkt)
{
    std::cout << "-----------qelar MACprepare send DATA_READY-----------\n";
    QELARHeader vbh;
    AquaSimTrailer tra;
...
void AquaSimQELAR::MACsend(Ptr<Packet> pkt, double delay)
{
    std::cout << "-----------qelar MACsend-----------\n";
...
double AquaSimQELAR::getlocalV(AquaSimAddress source)
{
    std::map<local_entry, qelar_neighbor *>::iterator it;
    double sourceV = 0.0;
    for (it = PktlocalTable.m_htable.begin(); it != PktlocalTable.m_htable.end(); it++)
...
void AquaSimQELAR::updateEnergyAfterSend(AquaSimAddress local)
{
    std::map<local_entry, qelar_neighbor *>::iterator it;
...
void AquaSimQELAR::updateAvgEnergyAfterrecv(AquaSimAddress local)
{
    std::map<local_entry, qelar_neighbor *>::iterator it;
    double LocalResiEnergy = GetNetDevice()->EnergyModel()->GetEnergy();
    for (it = PktlocalTable.m_htable.begin(); it != PktlocalTable.m_htable.end(); it++)
...
void AquaSimQELAR::printSourceV(int pktsendnum)
{
...
void AquaSimQELAR::updateDensity_delete_entry(AquaSimAddress local)
{
    std::map<local_entry, qelar_neighbor *>::iterator it;
    Vector p = GetNetDevice()->GetNode()->GetObject<MobilityModel>()->GetPosition();
...
void AquaSimQELAR::printEnergy(AquaSimAddress local, int pktID)
{
...
void AquaSimQELAR::iniNodetable()
{
    std::cout << "QELAR iniNodetable"
              << "\n";
...
void AquaSimQELAR::iniNodetable1()
{
    // 初始化自己信息
    // std::cout<<"QELAR iniNodetable1"<<"\n";
    AquaSimAddress localadd = AquaSimAddress::ConvertFrom(GetNetDevice()->GetAddress());
    // std::cout<<"localadd:"<<localadd<<"\n";
    Vector p = GetNetDevice()->GetPosition();
    double LocalResiEnergy = GetNetDevice()->EnergyModel()->GetEnergy();
    // std::cout<<"QELAR iniNodetable:"<<"localadd:"<<localadd<<","<<"LocalResiEnergy:"<<LocalResiEnergy<<"\n";
    PktlocalTable.PutInHash(localadd, 0, p, LocalResiEnergy,
                            LocalResiEnergy, 0, 0, 0);
    // 初始化邻居信息
    ...
    while (getline(neiborinfo, str))
    {
        ...
        if (iscloseenough(p, q) == 1 && current_node != localadd)
        {
            ...
            PktlocalTable.PutInHash(current_node, 0, q, LocalResiEnergy,
                                    LocalResiEnergy, 0, 0, 0);
            // std::cout<<"QELAR iniNodetable1 insert successful"<<"\n";
        }
    }
    neiborinfo.close();
    // 更新自己节点的密度，平均能量
...
int AquaSimQELAR::iscloseenough(Vector p, Vector q)
{
...
void AquaSimQELAR::DataForSink(Ptr<Packet> pkt)
{
    QELARHeader vbh;
    pkt->RemoveHeader(vbh);
    if (!SendUp(pkt))
...
void AquaSimQELAR::SetDelayTimer(Ptr<Packet> pkt, double c)
{
    NS_LOG_FUNCTION(this << c);
    if (c < 0)
        c = 0;
    Simulator::Schedule(Seconds(c), &AquaSimQELAR::Timeout, this, pkt);
}

void AquaSimQELAR::Timeout(Ptr<Packet> pkt)
{
    QELARHeader vbh;
...
double
AquaSimQELAR::Distance(Ptr<Packet> pkt) // d
{
    QELARHeader vbh;
    pkt->PeekHeader(vbh);
...
double
AquaSimQELAR::Advance(Ptr<Packet> pkt)
{
    QELARHeader vbh;
    pkt->PeekHeader(vbh);
...
double
AquaSimQELAR::Projection(Ptr<Packet> pkt, Vector p)
{
    QELARHeader vbh;
    AquaSimTrailer tra;
    pkt->RemoveTrailer(tra);
...
double AquaSimQELAR::SuccEnergy(unsigned int energy)
{
    double LocalResiEnergy = GetNetDevice()->EnergyModel()->GetEnergy();
...
double AquaSimQELAR::DefeatEnergy()
{
...
double AquaSimQELAR::SuccAvgEnergy(unsigned int energy, unsigned int avgenergy)
{
    std::map<local_entry, qelar_neighbor *>::iterator it;
    double AvgESi = 0.0;
    for (it = PktlocalTable.m_htable.begin(); it != PktlocalTable.m_htable.end(); it++)
...
double AquaSimQELAR::DefeatAvgEnergy()
{
    std::map<local_entry, qelar_neighbor *>::iterator it;
    double AvgESi = 0.0;
    for (it = PktlocalTable.m_htable.begin(); it != PktlocalTable.m_htable.end(); it++)
...
double AquaSimQELAR::Sendsucc(unsigned int sendnum, unsigned int sendsucc)
{
...
void AquaSimQELAR::printHopbyhopRecvdelay(AquaSimAddress local, AquaSimAddress previous, int pktid)
{
...
void AquaSimQELAR::printHopbyhopSenddelay(AquaSimAddress local, AquaSimAddress nexthop, int pktid)
{
    std::ofstream hopbyhop("Nodehopbyhop.txt", std::ios::app);
...
void AquaSimQELAR::printEndtoendRecvdelay(AquaSimAddress local, AquaSimAddress previous, int pktid)
{
    std::cout << "PktID:" << pktid << " Node:" << local << "\n";
}

void AquaSimQELAR::printEndtoendSenddelay(AquaSimAddress local, AquaSimAddress nexthop, int pktid)
{
    std::cout << "PktID:" << pktid << " Node:" << local << "\n";
}

/*
double AquaSimQELAR::SendDefeat(){

}
*/
bool AquaSimQELAR::IsTarget(Ptr<Packet> pkt)
{
    QELARHeader vbh;
    pkt->PeekHeader(vbh);
...
void AquaSimQELAR::DoDispose()
{
    m_rand = 0;
    AquaSimRouting::DoDispose();
}

void AquaSimQELAR::SetTargetPos(Vector pos)
{
    m_targetPos = pos;
}
