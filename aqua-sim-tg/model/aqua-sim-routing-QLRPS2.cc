...
#include "aqua-sim-routing-QLRPS2.h"
...
Vector allNodePosition2[18000][48];

NS_LOG_COMPONENT_DEFINE("AquaSimQLRPS22");
// NS_OBJECT_ENSURE_REGISTERED(AquaSimPktHashTable);
// todo add timeer
// clock_t start,finish;
AquaSimPktlocalTableQLRPS2::AquaSimPktlocalTableQLRPS2()
{
...
{
    NS_LOG_FUNCTION(this);
    for (std::map<local_entry, qlrp2_neighbor *>::iterator it = m_htable.begin(); it != m_htable.end(); ++it)
    {
        delete it->second;
    }
    m_htable.clear();
}
void AquaSimPktlocalTableQLRPS2::Reset()
{
    m_htable.clear();
}
qlrp2_neighbor *
AquaSimPktlocalTableQLRPS2::GetHash(AquaSimAddress forwarderAddr, Vector p)
{
    // std::cout<<"++++++GetHash+++++++++++++\n";
    local_entry entry = std::make_pair(forwarderAddr, p);

    std::map<local_entry, qlrp2_neighbor *>::iterator it;
...
void AquaSimPktlocalTableQLRPS2::PutInHash(AquaSimAddress fAddr, double Vvalue, Vector p, unsigned int Energy, unsigned int Avgenergy, unsigned int Density, unsigned int sendnum, unsigned int sendsucc)
{
    ...
    /*for(it=m_htable.begin();it!=m_htable.end();it++){
        ...
        if(cur.first==fAddr){
          ...
          unsigned int sndsucc=nei->sendsucc;
           std::map<local_entry,qlrp2_neighbor*>::iterator iter;
    ...
    std::pair<local_entry, qlrp2_neighbor *> newPair;
    newPair.first = entry;
...
void AquaSimPktlocalTableQLRPS2::UpdateHash(AquaSimAddress local, AquaSimAddress nexthop, double Vvalue)
{ // 更新V值和Sendnum
    std::map<local_entry, qlrp2_neighbor *>::iterator it;
    for (it = m_htable.begin(); it != m_htable.end(); it++)
    {
        local_entry update = it->first;
        qlrp2_neighbor *neibor = it->second;
...
void AquaSimPktlocalTableQLRPS2::UpdateSuccnum(AquaSimAddress previous, AquaSimAddress forward)
{
    std::map<local_entry, qlrp2_neighbor *>::iterator it;
    for (it = m_htable.begin(); it != m_htable.end(); it++)
    {
...
void AquaSimPktlocalTableQLRPS2::updateOwn(double avgengy, double denty, AquaSimAddress cur)
{
    std::map<local_entry, qlrp2_neighbor *>::iterator it;
    for (it = m_htable.begin(); it != m_htable.end(); it++)
    {
...
AquaSimDataHashTableSQLRPS2 ::AquaSimDataHashTableSQLRPS2()
{
    NS_LOG_FUNCTION(this);
    Reset();
...
void AquaSimDataHashTableSQLRPS2 ::Reset()
{
...
int *AquaSimDataHashTableSQLRPS2 ::GetHash(int *attr)
{
...
void AquaSimDataHashTableSQLRPS2 ::PutInHash(int *attr)
{
    // bool newPtr = true;
...
NS_OBJECT_ENSURE_REGISTERED(AquaSimQLRPS2);

AquaSimQLRPS2::AquaSimQLRPS2()
{
    ...
    /*Simulator::Schedule(Seconds(0.5),
                 &AquaSimQLRPS2::iniNodetable, this);*/
    Simulator::Schedule(Seconds(0.5),
                        &AquaSimQLRPS2::iniNodetable1, this);
    /*Simulator::Schedule(Seconds(10),
                  &AquaSimQLRPS2::PrepareMessage, this);*/
}

TypeId
AquaSimQLRPS2::GetTypeId(void)
{
    static TypeId tid = TypeId("ns3::AquaSimQLRPS2")
                            .SetParent<AquaSimRouting>()
                            .AddConstructor<AquaSimQLRPS2>()
                            .AddAttribute("HopByHop", "Hop by hop QLRP setting. Default 0 is false.",
                                          IntegerValue(0),
                                          MakeIntegerAccessor(&AquaSimQLRPS2::m_hopByHop),
                                          MakeIntegerChecker<int>())
                            .AddAttribute("EnableRouting", "Enable routing QLRP setting. Default 1 is true.",
                                          IntegerValue(1),
                                          MakeIntegerAccessor(&AquaSimQLRPS2::m_enableRouting),
                                          MakeIntegerChecker<int>())
                            .AddAttribute("Width", "Width of QLRPS. Default is 100.",
                                          DoubleValue(3000),
                                          // DoubleValue(100),
                                          MakeDoubleAccessor(&AquaSimQLRPS2::m_width),
                                          MakeDoubleChecker<double>())
                            .AddAttribute("TargetPos", "Position of target sink (x,y,z).",
                                          Vector3DValue(),
                                          MakeVector3DAccessor(&AquaSimQLRPS2::m_targetPos),
                                          MakeVector3DChecker());
    return tid;
...
int64_t
AquaSimQLRPS2::AssignStreams(int64_t stream)
{
...
// 传输层调用Txprocess
bool AquaSimQLRPS2::TxProcess(Ptr<Packet> packet, const Address &dest, uint16_t protocolNumber)
{
    QLRPSHeader vbh;
    // start=clock();
...
// MAC层的包调用Recv
bool AquaSimQLRPS2::Recv(Ptr<Packet> packet, const Address &dest, uint16_t protocolNumber)
{
    NS_LOG_FUNCTION(this);
    std::cout << "================qlrp Recv=================\n";
    ...
    if (vbh.GetMessType() == DATA_READY)
    {
        ...
        // qlrp2_neighbor *hash=PktlocalTable.GetHash(vbh.GetSenderAddr(),vbh.GetExtraInfo().f);
        // todo
    ...
    else
    {
        ...
        qlrp2_neighbor *hash = PktlocalTable.GetHash(vbh.GetForwardAddr(), vbh.GetExtraInfo().f);
        // todo
...
void AquaSimQLRPS2::ConsiderNew(Ptr<Packet> pkt)
{
    ...
    NS_LOG_INFO("AquaSimQLRPS2::ConsiderNew: data packet");
    // todo 打印每个节点的剩余能量
    ...
    if (GetNetDevice()->GetAddress() == from_nodeAddr)
    {
        ...
        // MACprepareF(pkt);
        Simulator::Schedule(Seconds(0.5),
                            &AquaSimQLRPS2::MACprepareF, this, pkt);
        // MACsend(pkt,0.5);//macsend比Macprepare提前执行了
        Simulator::Schedule(Seconds(0.5), &AquaSimQLRPS2::MACsend, this, pkt, 0.5);
        // 发送完后更新自己的表中能量和平均能量值
        // todo
        // updateEnergyAfterSend(AquaSimAddress::ConvertFrom(GetNetDevice()->GetAddress()));
        Simulator::Schedule(Seconds(0.5), &AquaSimQLRPS2::updateEnergyAfterSend, this,
                            AquaSimAddress::ConvertFrom(GetNetDevice()->GetAddress()));
        // 把source节点的V值单独打印log
        Simulator::Schedule(Seconds(0.5),
                            &AquaSimQLRPS2::printSourceV, this, m_pkCount);
        // printSourceV();
        //}
        return;
    }
...
void AquaSimQLRPS2::Terminate()
{
    NS_LOG_DEBUG("AquaSimQLRPS2::Terminate: Node=" << GetNetDevice()->GetAddress() << ": remaining energy=" << GetNetDevice()->EnergyModel()->GetEnergy() << ", initial energy=" << GetNetDevice()->EnergyModel()->GetInitialEnergy());
}

void AquaSimQLRPS2::StopSource()
{
    /*
       Agent_List *cur;
...
void AquaSimQLRPS2::PrepareMessage()
{
    ...
    {
        // std::pair<local_entry,qlrp2_neighbor*> curpair;
    ...
    Simulator::Schedule(Seconds(65.0),
                        &AquaSimQLRPS2::PrepareMessage, this); // 定期调用自己
                                                               // MACprepare(pkt);
                                                               // return pkt;
}

void AquaSimQLRPS2::MACprepareF(Ptr<Packet> pkt)
{
    ...
    for (it = PktlocalTable.m_htable.begin(); it != PktlocalTable.m_htable.end(); it++)
    {
        ...
        local_entry curentry = it->first;
        qlrp2_neighbor *neibor = it->second;
        int nodeid = curentry.first.GetAsInt();
        ...
        else
        {
            ...
            double d = Projection(pkt, getPosition(t, nodeid));
...
void AquaSimQLRPS2::MACprepare(Ptr<Packet> pkt)
{
...
void AquaSimQLRPS2::MACsend(Ptr<Packet> pkt, double delay)
{
...
double AquaSimQLRPS2::getlocalV(AquaSimAddress source)
{
    std::map<local_entry, qlrp2_neighbor *>::iterator it;
    double sourceV = 0.0;
    for (it = PktlocalTable.m_htable.begin(); it != PktlocalTable.m_htable.end(); it++)
...
void AquaSimQLRPS2::updateEnergyAfterSend(AquaSimAddress local)
{
    std::map<local_entry, qlrp2_neighbor *>::iterator it;
...
void AquaSimQLRPS2::updateAvgEnergyAfterrecv(AquaSimAddress local)
{
    std::map<local_entry, qlrp2_neighbor *>::iterator it;
    double LocalResiEnergy = GetNetDevice()->EnergyModel()->GetEnergy();
    for (it = PktlocalTable.m_htable.begin(); it != PktlocalTable.m_htable.end(); it++)
...
void AquaSimQLRPS2::printSourceV(int pktsendnum)
{
...
void AquaSimQLRPS2::updateDensity_delete_entry(AquaSimAddress local)
{
    std::map<local_entry, qlrp2_neighbor *>::iterator it;
    Vector p = GetNetDevice()->GetNode()->GetObject<MobilityModel>()->GetPosition();
    int t = Simulator::Now().GetSeconds();
    ...
    for (it = PktlocalTable.m_htable.begin(); it != PktlocalTable.m_htable.end(); it++)
    {
        ...
        if (curentry.first != local && iscloseenough(p, getPosition(t, nodeid)) == 1)
        {
            // std::cout<<"iscloseenough!"<<"\n";
            continue;
        }
        else if (curentry.first != local && iscloseenough(p, getPosition(t, nodeid)) == 0)
        {
            std::map<local_entry, qlrp2_neighbor *>::iterator iter;
...
void AquaSimQLRPS2::printEnergy(AquaSimAddress local, int pktID)
{
...
void AquaSimQLRPS2::iniNodetable()
{
...
void AquaSimQLRPS2::iniNodetable1()
{
    // 初始化自己信息
    ...
    for (int i = 1; i < 48; i++)
    {
        if (i != localadd.GetAsInt())
        {
            ...
            PktlocalTable.PutInHash(i, 0, getPosition(t, i), LocalResiEnergy,
                                    LocalResiEnergy, 0, 0, 0);
...
int AquaSimQLRPS2::iscloseenough(Vector p, Vector q)
{
...
void AquaSimQLRPS2::DataForSink(Ptr<Packet> pkt)
{
    QLRPSHeader vbh;
    pkt->RemoveHeader(vbh);
    if (!SendUp(pkt))
...
void AquaSimQLRPS2::SetDelayTimer(Ptr<Packet> pkt, double c)
{
    NS_LOG_FUNCTION(this << c);
    if (c < 0)
        c = 0;
    Simulator::Schedule(Seconds(c), &AquaSimQLRPS2::Timeout, this, pkt);
}

void AquaSimQLRPS2::Timeout(Ptr<Packet> pkt)
{
    QLRPSHeader vbh;
...
double
AquaSimQLRPS2::Distance(Ptr<Packet> pkt) // d
{
    QLRPSHeader vbh;
    pkt->PeekHeader(vbh);
...
double
AquaSimQLRPS2::Advance(Ptr<Packet> pkt)
{
    QLRPSHeader vbh;
    pkt->PeekHeader(vbh);
...
double
AquaSimQLRPS2::Projection(Ptr<Packet> pkt, Vector p)
{
    QLRPSHeader vbh;
    AquaSimTrailer tra;
    pkt->RemoveTrailer(tra);
...
double AquaSimQLRPS2::SuccEnergy(unsigned int energy)
{
    double LocalResiEnergy = GetNetDevice()->EnergyModel()->GetEnergy();
...
double AquaSimQLRPS2::DefeatEnergy()
{
...
double AquaSimQLRPS2::SuccAvgEnergy(unsigned int energy, unsigned int avgenergy)
{
    std::map<local_entry, qlrp2_neighbor *>::iterator it;
    double AvgESi = 0.0;
    for (it = PktlocalTable.m_htable.begin(); it != PktlocalTable.m_htable.end(); it++)
...
double AquaSimQLRPS2::DefeatAvgEnergy()
{
    std::map<local_entry, qlrp2_neighbor *>::iterator it;
    double AvgESi = 0.0;
    for (it = PktlocalTable.m_htable.begin(); it != PktlocalTable.m_htable.end(); it++)
...
double AquaSimQLRPS2::Sendsucc(unsigned int sendnum, unsigned int sendsucc)
{
...
void AquaSimQLRPS2::printHopbyhopRecvdelay(AquaSimAddress local, AquaSimAddress previous, int pktid)
{
...
void AquaSimQLRPS2::printHopbyhopSenddelay(AquaSimAddress local, AquaSimAddress nexthop, int pktid)
{
    std::ofstream hopbyhop("Nodehopbyhop.txt", std::ios::app);
...
void AquaSimQLRPS2::printEndtoendRecvdelay(AquaSimAddress local, AquaSimAddress previous, int pktid)
{
    std::cout << "PktID:" << pktid << " Node:" << local << "\n";
}

void AquaSimQLRPS2::printEndtoendSenddelay(AquaSimAddress local, AquaSimAddress nexthop, int pktid)
{
    std::cout << "PktID:" << pktid << " Node:" << local << "\n";
}

/*
double AquaSimQLRPS2::SendDefeat(){

}
*/
bool AquaSimQLRPS2::IsTarget(Ptr<Packet> pkt)
{
    QLRPSHeader vbh;
    pkt->PeekHeader(vbh);
...
void AquaSimQLRPS2::DoDispose()
{
    m_rand = 0;
    AquaSimRouting::DoDispose();
}

void AquaSimQLRPS2::SetTargetPos(Vector pos)
{
    m_targetPos = pos;
}
Vector AquaSimQLRPS2::getPosition(int i, int nodeid)
{
    return allNodePosition2[i][nodeid - 1];
}