  ...
  m_statisticalIteration(0)
{
  ...
  m_ddosCheckFrequency = Minutes(0.00001);//Minutes(10);
...
AquaSimDDOS::RecvNAck(Ptr<Packet> p) //NOTE TODO: should be called by phy layer in case that BER/SINR is too high.
{
 ...
 // it = DdosDetectionTable.find(ash.GetSAddr().GetAsInt());
   it = DdosDetectionTable.find(tra.GetSAddr().GetAsInt());
...
{
	std::cout<<"============AquaSimDDOS::SendInteest=================\n";
...
{
	std::cout<<"============AquaSimDDOS::SendAlert=================\n";
  ...
  ash.SetNumForwards(1);*/
  tra.SetSAddr(AquaSimAddress::ConvertFrom(GetNetDevice()->GetAddress()));
  tra.SetDAddr(AquaSimAddress (attackerAddr));
  tra.SetNextHop(AquaSimAddress::GetBroadcast());
...
//传输层调用Txprocess
bool AquaSimDDOS::TxProcess(Ptr< Packet > packet, const Address &dest, uint16_t protocolNumber)
{
  DDOSHeader ddo;
  
  ddo.SetRowIndex(GetNetDevice()->GetIfIndex());
	packet->AddHeader(ddo);
...
packet->RemoveHeader(dh);
 /* std::cout<<"AquaSimDDOS::Recv ash.GetNumForwards():"<<ash.GetNumForwards()<<"\n";
  std::cout<<"AquaSimDDOS::Recv ash.GetUId():"<<ash.GetUId()<<"\n";
  std::cout<<"AquaSimDDOS::Recv ash.GetSAddr():"<<ash.GetSAddr()<<"\n";
  std::cout<<"AquaSimDDOS::Recv ash.GetDAddr():"<<ash.GetDAddr()<<"\n";*/
  std::cout<<"AquaSimDDOS::Recv GetNetDevice()->GetAddress():"<<GetNetDevice()->GetAddress()<<"\n";
  if (tra.GetNextHop()!=AquaSimAddress::GetBroadcast())  //no headers
  {
	std::cout<<"AquaSimDDOS::Recv no headers\n";
  ...
  {
	std::cout<<"AquaSimDDOS::Recv have headers\n";
    NS_LOG_DEBUG("RECV: updating packet.");
    if(isAttacker)
      {
    	//std::cout<<"AquaSimDDOS::Recv Attacker received a packet, dropping.\n";
  ...
  it=DdosDetectionTable.find(tra.GetSAddr().GetAsInt());
  if (it == DdosDetectionTable.end()) {
	std::cout<<"AquaSimDDOS::Recv No id entry in DdosDetectionTable exists, creating new\n";
    NS_LOG_DEBUG("No id entry in DdosDetectionTable exists, creating new.");
    DdosDetectionTable.insert( std::pair<int, DdosTable>(tra.GetSAddr().GetAsInt(),
                                          DdosTable(tra.GetSAddr().GetAsInt())));
    //todo
    it = DdosDetectionTable.find(tra.GetSAddr().GetAsInt());
  }
  (it->second).idle=false;
  std::cout<<"111111AquaSimDDOS::Recv m_totalPktRecv:"<<m_totalPktRecv<<" ,(it->second).pktRecv:"<<(it->second).pktRecv<<"\n";
  //todo
/*
  if(!NodeContainsDataPath(interest) && dh.GetPacketType() != DDOSHeader::Alert)
  {
	std::cout<<"AquaSimDDOS::Recv No Data Path found on node("<< GetNetDevice()->GetAddress() <<") from node " << ash.GetSAddr() << " for interest:" << interest;
  ...
  if ((it->second).pushback.IsRunning() || (it->second).throttle.IsRunning()) {
	std::cout<<"AquaSimDDOS::Recv Potential attack ignored from node:"<<tra.GetSAddr()<<"\n";
    //NS_LOG_INFO("Potential attack ignored from node:" << ash.GetSAddr());
    std::cout<<"Before AquaSimDDOS::Recv m_totalPktRecv:"<<m_totalPktRecv<<" ,(it->second).pktRecv:"<<(it->second).pktRecv<<"\n";
    m_totalPktRecv++;
    (it->second).pktRecv++;
    std::cout<<"After AquaSimDDOS::Recv m_totalPktRecv:"<<m_totalPktRecv<<" ,(it->second).pktRecv:"<<(it->second).pktRecv<<"\n";
    //return false;
  }
  ...
  if (dh.GetPacketType() == DDOSHeader::Interest) {
	std::cout<<"Recv Interest Pkt recved on node:"<<GetNetDevice()->GetAddress()<<"\n";
    ...
    {
      std::cout<<"Recv dh.GetPacketType() == DDOSHeader::Interest,Suppressing back stream effect of interest packet\n";
    ...
    if (NodeContainsRelatedData(interest)) {
      std::cout<<"Recv dh.GetPacketType() == DDOSHeader::Interest,Related Data found for interest\n";
    ...
    if (!m_data.empty())
    {
      //is a data source and should not be retransmitting interests//bi mian chong chuan related shuju
      std::cout<<"Recv DDosHeader::interest Data Source:"<< GetNetDevice()->GetAddress() <<"does not contain related data and therefore is dropping packet\n";
    ...
    if (entryFound) {
      std::cout<<"Recv DDosHeader::interest PIT entry found for interest:"<< interest << " | Dropping pkt.\n";
      NS_LOG_DEBUG("PIT entry found for interest:" << interest << " | Dropping pkt.");
      std::cout<<"Before Recv DDosHeader::interest PIT entry not found for interest timeout ratio:"<<(it->second).interestEntries<<"\n";
      //interest entry found in PIT
      (it_pit->second).nodeID.insert(tra.GetSAddr().GetAsInt());
      (it->second).interestEntries++;//timeout ratio++
      std::cout<<"After Recv DDosHeader::interest PIT entry found for interest timeout ratio:"<<(it->second).interestEntries<<"\n";
      //packet dropped
      return true;
    }
    else {
      std::cout<<"Recv ddosheader::interest Creating new entry in PIT for interest:\n";
      std::cout<<"Before Recv DDosHeader::interest PIT entry not found for interest timeout ratio:"<<(it->second).interestEntries<<"\n";
      ...
      std::set<int> nodeIDs;
      nodeIDs.insert(tra.GetSAddr().GetAsInt());
      ...
      std::cout<<"Recv DDosHeader::interest PIT entry not found for interest interestEntries:"<<(it->second).interestEntries<<"\n";
      std::cout<<"Recv DDosHeader::interest PIT entry not found for interest entriesTimeout:"<<(it->second).entriesTimeout<<"\n";
      SendInterest(packet);
	...
	std::cout<<"Recv dh.GetPacketType() == DDOSHeader::Data\n";
    ...
    if (entryFound) {
      if(GetNetDevice()->GetSinkStatus()) {
    	...
    	std::cout<<"sink:"<< GetNetDevice()->GetAddress() <<" recved interest:"<<interest<<"\n";
        NS_LOG_INFO("     Sink(" << GetNetDevice()->GetAddress() << ") recved interest:"
            << interest << " at time " << Simulator::Now().ToDouble(Time::S));
      }
      else {
    	std::cout<<"Recv dh.GetPacketType() == DDOSHeader::Data SendData\n";
        SendData(packet);
      }
      //RemoveEntry(interest,false);
      RemoveEntry(interest,true);
      return true;
    }
	...
	std::cout<<"Alert Pkt recved on node: "<< GetNetDevice()->GetAddress() << " for node " << tra.GetDAddr()<<"\n";
    NS_LOG_DEBUG("Alert Pkt recved on node " << GetNetDevice()->GetAddress() << " for node " << tra.GetDAddr());

    std::map<int, DdosTable>::iterator attackerIter;
    attackerIter = DdosDetectionTable.find(tra.GetDAddr().GetAsInt());
...
AquaSimDDOS::NodeContainsDataPath(std::string interest)
{
  ...
  std::cout<<"DataPath for interest: "<<interest << " is: "<< path<<std::endl;
  ...
  for (uint i=0; i < m_knownDataPath.size(); i++)
  {
    ...
    std::cout<<"m_knownDataPath "<<i << " : "<< m_knownDataPath[i]<<std::endl;
  }
  return false;
}
...
AquaSimDDOS::RemoveEntry(std::string interest, bool isTimeout)
{
  ...
  std::cout<<"DDOS RemoveEntry: "<<GetNetDevice()->GetAddress() << interest << isTimeout<<std::endl;
  std::map<std::string, PIT_info>::iterator it_pit;
  it_pit = PIT.find(interest);
  if (it_pit == PIT.end()) {
  ...
  if(isTimeout) {
	std::cout<<"DDOS isTimeout: "<<GetNetDevice()->GetAddress() << interest << isTimeout<<std::endl;
...
{
  //std::cout<<"============AquaSimDDOS::DdosAttackCheck()===========\n";
  ...
  for (; it!=DdosDetectionTable.end(); it++) {
    ...
    std::cout<<"DdosCheckAtt (localDdosTable).entriesTimeout:"<<(localDdosTable).entriesTimeout<<",  (localDdosTable).interestEntries:"<<(localDdosTable).interestEntries<<"\n";
    ...
    std::cout<<"DdosCheckAtt timeoutRatio:"<<timeoutRatio<<"  channelMeasurement:"<<channelMeasurement<<"  transDominance"<<transDominance<<" pitUsage: "<<pitUsage<<"\n";
    ...
    std::cout<<"DdosCheckAttack (1+channelMeasurement)*timeoutRatio:"<<(1+channelMeasurement)*timeoutRatio<<"\n";
    std::cout<<"DdosCheckAttack (localDdosTable).timeoutThreshold:"<<(localDdosTable).timeoutThreshold<<"\n";
    std::cout<<"DdosCheckAttack (localDdosTable).pbThreshold):"<<localDdosTable.pbThreshold<<"\n";
    std::cout<<"DdosCheckAttack (localDdosTable).throttleThreshold):"<<localDdosTable.throttleThreshold<<"\n";
    ...
    if (!potentialAttack && ((1+channelMeasurement)*timeoutRatio >= (localDdosTable).timeoutThreshold)  &&
            (transDominance >= (localDdosTable).throttleThreshold) )
      {
        ...
        potentialAttack=true;
        /* (idea here is if throttling isn't detected quickly it can lead to multiple throttles downstream, which will eventually reset once the entry node attackee fully throttles the attacker.) */
      //}*/
    if((1+channelMeasurement)*timeoutRatio>=(localDdosTable).timeoutThreshold){
    	if(pitUsage>=(localDdosTable).pbThreshold){
    		std::cout << Simulator::Now().ToDouble(Time::S) << " Pushback(" << GetNetDevice()->GetAddress() << ") for " << localNodeId << "\n";
    		Pushback(localNodeId);
    		potentialAttack=true;
    	}
    	//todo
    	//if(transDominance >= (localDdosTable).throttleThreshold){
    	if(!potentialAttack&&(transDominance >= (localDdosTable).throttleThreshold)){
    		std::cout << Simulator::Now().ToDouble(Time::S) << " Throttle(" << GetNetDevice()->GetAddress() << ") for " << localNodeId << "\n";
    		Throttle(localNodeId);
    		potentialAttack=true;
