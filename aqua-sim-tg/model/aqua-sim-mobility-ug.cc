...
NS_LOG_COMPONENT_DEFINE ("UnderwaterGilderMobilityModel");

NS_OBJECT_ENSURE_REGISTERED (UnderwaterGilderMobilityModel);

TypeId
UnderwaterGilderMobilityModel::GetTypeId (void)
{
    static TypeId tid = TypeId ("ns3::UnderwaterGilderMobilityModel")
        .SetParent<MobilityModel> ()
        .SetGroupName ("Mobility")
        .AddConstructor<UnderwaterGilderMobilityModel> ()
        .AddAttribute ("VariableSpeed", "Set gilder speed is Variable. Default false.",
                        BooleanValue(0),
                        MakeBooleanAccessor (&UnderwaterGilderMobilityModel::m_isVarSpeed),
                        MakeBooleanChecker())
        .AddAttribute ("Bounds", "The 2d bounding area",
                        RectangleValue (Rectangle (-100, 100, -100, 100)),
                        MakeRectangleAccessor (&UnderwaterGilderMobilityModel::m_bounds),
                        MakeRectangleChecker ())
        .AddAttribute ("OpenAngle", "UnderwaterGilder's open angle(degree)",
                        StringValue ("ns3::UniformRandomVariable[Min=30.0|Max=50.0]"),
                        MakePointerAccessor (&UnderwaterGilderMobilityModel::m_openAngle),
                        MakePointerChecker<UniformRandomVariable> ())
        .AddAttribute ("Depth", "UnderwaterGilder's depth range",
                        StringValue ("ns3::UniformRandomVariable[Min=100.0|Max=1000.0]"),
                        MakePointerAccessor (&UnderwaterGilderMobilityModel::m_depthRange),
                        MakePointerChecker<UniformRandomVariable> ())
        ...
        .AddAttribute ("Speed", "A random variable to control the speed (m/s).",
                        ...
                        MakePointerAccessor (&UnderwaterGilderMobilityModel::m_rndSpeed),
                        MakePointerChecker<UniformRandomVariable> ())
        .AddAttribute ("Pause", "A random variable to control the pause (s).",
                        StringValue ("ns3::ConstantRandomVariable[Constant=0.1]"),
                        MakePointerAccessor (&UnderwaterGilderMobilityModel::m_pause),
        ...
        .AddAttribute ("Direction",
                        ...
                        MakePointerAccessor (&UnderwaterGilderMobilityModel::m_rndDirection),
                        MakePointerChecker<UniformRandomVariable> ())
    ;
    return tid;
}

UnderwaterGilderMobilityModel::UnderwaterGilderMobilityModel ()
{
    NS_LOG_FUNCTION(this);
    Simulator::ScheduleNow(&UnderwaterGilderMobilityModel::Init, this);
}
...
void
UnderwaterGilderMobilityModel::Init()
{
    NS_LOG_FUNCTION(this);
    m_speed = 0.0;
    m_direction = 0.0;
    ...
    m_event = Simulator::ScheduleNow(&UnderwaterGilderMobilityModel::Start, this);
    m_helper.Unpause();
}
void 
UnderwaterGilderMobilityModel::DoDispose (void)
{
  MobilityModel::DoDispose ();
}

void
UnderwaterGilderMobilityModel::DoInitialize (void)
{
...
Vector
UnderwaterGilderMobilityModel::DoGetPosition (void) const
{
  m_helper.UpdateWithBounds (m_bounds);
  return m_helper.GetCurrentPosition ();
}
void
UnderwaterGilderMobilityModel::DoSetPosition (const Vector &position)
{
...
Vector
UnderwaterGilderMobilityModel::DoGetVelocity (void) const
{
  return m_helper.GetVelocity ();
}
int64_t
UnderwaterGilderMobilityModel::DoAssignStreams (int64_t stream)
{
...
void 
UnderwaterGilderMobilityModel::Start (void)
{
    NS_LOG_FUNCTION(this);
    if (m_speed == 0.0) {
...
void 
UnderwaterGilderMobilityModel::DoWalk (Time delayLeft)
{
    ...
    m_event = Simulator::Schedule(delayLeft, &UnderwaterGilderMobilityModel::Start, this);
