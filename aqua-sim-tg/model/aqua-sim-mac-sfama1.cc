...
AquaSimSFama::AquaSimSFama() :
		m_status(IDLE_WAIT), m_guardTime(0.00001), m_slotLen(0), m_isInRound(
				false), m_isInBackoff(false), m_maxBackoffSlots(4), m_maxBurst(
				1), m_dataSendingInterval(0.0000001), m_waitSendTimer(this), m_waitReplyTimer(
...
TypeId AquaSimSFama::GetTypeId(void) {
	static TypeId tid =
			TypeId("ns3::AquaSimSFama").SetParent<AquaSimMac>().AddConstructor<
					...
					"The guard time in double. Default is 0.00001",
					DoubleValue(0.00001),
...
bool AquaSimSFama::RecvProcess(Ptr<Packet> p) {
	...
	NS_LOG_DEBUG(
			"Time:" << Simulator::Now().GetSeconds() << ",node:" << m_device->GetNode() << ",node " << mach.GetDA() << " recv from node " << mach.GetSA());

	switch (SFAMAh.GetPType()) {
	case SFamaHeader::SFAMA_RTS:
		std::cout << "RecvRTS" << std::endl;
		ProcessRTS(p);
		break;
	case SFamaHeader::SFAMA_CTS:
		std::cout << "RecvCTS" << std::endl;
		ProcessCTS(p);
		break;
...
void AquaSimSFama::InitSlotLen() /// slot = guardTime + trans + TXtime of CTS
{
	SFamaHeader SFAMA;
	m_slotLen = m_guardTime + GetTxTime(SFAMA.GetSize(SFamaHeader::SFAMA_CTS)
...
void AquaSimSFama::ProcessCTS(Ptr<Packet> cts_pkt) {
	...
	if (mach.GetDA() == AquaSimAddress::ConvertFrom(m_device->GetAddress()))
		...
		double wait_time = (1 + SFAMAh.GetSlotNum()) * m_slotLen
				+ time2comingslot;
		...
		//do backoff
		double backoff_time = SFAMAh.GetSlotNum() * m_slotLen /*for data*/+ 1 /*for ack*/
		+ time2comingslot;
...
void AquaSimSFama::ProcessDATA(Ptr<Packet> data_pkt) {
	...
	if (mach.GetDA() == AquaSimAddress::ConvertFrom(m_device->GetAddress())
		...
		//do backoff
		double backoff_time = 1
				+ GetTime2ComingSlot(
						Simulator::Now().ToDouble(Time::S)) /*for ack*/;
...
void AquaSimSFama::PrepareSendingDATA() {
    ...
    NS_LOG_DEBUG(PrintAllQ());
		...
		SFAMAh_tmp.SetPType(SFamaHeader::SFAMA_DATA);
		mach_tmp.SetDA(ash_tmp.GetNextHop());
		mach_tmp.SetSA(AquaSimAddress::ConvertFrom(m_device->GetAddress()));

		ash_tmp.SetDirection(AquaSimHeader::DOWN);
		tmp_pkt->AddHeader(mach_tmp);
  ...
  NS_LOG_DEBUG(PrintAllQ());
	...
	double additional_txtime = GetPktTrainTxTime()
			- GetTxTime(SFAMAh.GetSize(SFamaHeader::SFAMA_CTS)).ToDouble(
					Time::S);

	ScheduleRTS(recver_addr, (additional_txtime / m_slotLen) + 1 /*for ceil*/
			+ 1/*the basic slot*/); //////////////////////////////???????????????basic slot;
}



double AquaSimSFama::GetPktTrainTxTime() { /// calculate Txtime of sendPktQ
	double txtime = 0.0;
...
void AquaSimSFama::SendPkt(Ptr<Packet> pkt) {
	...
	case NIDLE:
		...
		pkt->AddHeader(mach);
		NS_LOG_DEBUG(
				Simulator::Now().GetSeconds() << ": node " << mach.GetSA() << " send to node " << mach.GetDA());
		std::cout << "senddown\n";
		SendDown(pkt);
...
void AquaSimSFama::WaitSendTimerProcess(Ptr<Packet> pkt) { /////send RTSpkt
	if (NULL == pkt) {
